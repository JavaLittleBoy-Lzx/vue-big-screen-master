...
1. **检查后端服务**
   ...
   # 检查后端服务是否运行
   curl http://10.100.111.2:8675/parking/nefuData/testWebSocketPush?type=in&plateNumber=测试
   ```
...
```bash
   # 测试车辆进场推送
   curl -X POST "http://10.100.111.2:8675/parking/nefuData/testWebSocketPush?type=in&plateNumber=京A12345"

   # 测试车辆离场推送
   curl -X POST "http://10.100.111.2:8675/parking/nefuData/testWebSocketPush?type=out&plateNumber=京A12345"
```
